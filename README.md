...
2、yq 二进制改为固定版本 + 同 release 下 SHA-256 校验,失败直接中止

输入注入 / 机密保护
3、新增 isValidDomain;自签证书 openssl 全部参数加引号,修复 printf 格式串隐患
4、SOCKS5 出站 yq eval 改用 strenv() 绑定环境变量,杜绝 YAML 注入
5、auth_secret、socks5 密码、6 个 ACME DNS API token 改 read -rs 不回显;认证口令不再以明文 echo
6、ACL 添加/删除改为 grep -Fxq 整行精确匹配,消除 sed 通配符注入
7、delPortHoppingNat 删除 eval,改用 xargs 安全转义
8、umask 077 + chmod 700/600 收紧 /etc/hihy 内机密文件权限

文件竞态 / 运行时硬化
9、mikutap 包、crontab 缓冲、hihy_debug.info 改用 mktemp 或 root-only 目录
10、service 脚本 stop() 校验 /proc/$PID/comm == appS 再 kill;PID 文件 chmod 600
11、新增 isValidPort,端口输入要求 1-65535 整数;随机端口源改 /dev/urandom
12、脚本顶部显式设置 PATH 以防环境劫持

默认行为调整
13、masquerade.proxy.insecure 默认改为 false;新增交互询问是否跳过上游 TLS 校验
14、伪装字符串 / content-stuff 默认值改为安装期随机生成,抗指纹
15、ACME 流程提示:若域名挂 Cloudflare 请保持"仅 DNS"灰云,橙云代理不支持 UDP/QUIC
16、菜单与脚本版本号更新为 ver1.04-d
```
...
**脚本每次更新都可能会发生改变,具体交互文案、步骤序号、默认值等以脚本实际运行为准。** 若遇到问题先查看 [常见问题](md/issues.md) 与 [历史改进](md/logs.md),仍无法定位再提 issue。

<details>
  <summary>(已折叠) 旧版交互演示截图,仅供参考</summary>
<pre><blockcode> 
